...
export const Home: React.FC = () => {
	...
	useEffect(() => {
		// Inicializar el carousel solo cuando el componente esté montado
		if (carouselRef.current && window.bootstrap?.Carousel) {
			const carouselElement = carouselRef.current;
			// Verificar si ya existe una instancia del carousel
			let carouselInstance = window.bootstrap.Carousel.getInstance(carouselElement);
			
			if (!carouselInstance) {
				// Crear nueva instancia del carousel
			...
			// Cleanup: destruir el carousel cuando el componente se desmonte
			return () => {
				if (carouselInstance) {
					try {
						...
						console.warn('Error disposing carousel:', error);
					}
				}
			};
		}
